...
class MainWindow(ttk.Frame):  # Change to inherit from ttk.Frame
    ...
    def add_survey(self):
        # Modificar para pasar self en lugar de self.root como parent
        survey_form = SurveyForm(self.root)
        # Esperar a que se cierre la ventana y recargar datos
        self.wait_window(survey_form)
        self.load_data()
    ...
    def edit_survey(self):
        ...
        survey = self.dao.read_by_id(values[0])
        if survey:
            survey_form = SurveyForm(self.root, survey)
            self.wait_window(survey_form)
            self.load_data()
